...
Multi-user SaaS with PostgreSQL + email/password auth
"""
import os
import logging
...
# ── Core components ────────────────────────────────────────────
# State manager holds ONLY shared scan data (opportunities, market data).
# All per-user data (positions, history, config) lives in PostgreSQL.
...
# ── Database + Auth (SaaS mode — required) ────────────────────
db_enabled = False
...
if not db_enabled:
    log.warning("DB not configured — set USE_DB=true and DATABASE_URL for full functionality")

# Give scanner access to flask app for DB operations
if db_enabled:
...
log.info(
    ...
    f"DB: {'connected' if db_enabled else 'not configured'}"
)
