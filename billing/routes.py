...
from billing.polar_client import (
...
def billing_status():
    ...
    return jsonify({
        ...
        'has_portal': bool(getattr(current_user, 'provider_customer_id', None)),
        'limits': limits,
        'plans': {k: {
            'label': v['label'],
...
def billing_checkout():
    ...
    try:
        ...
        logger.error("Polar checkout error: %s", e)
...
def billing_webhook():
    ...
    webhook_secret = current_app.config.get('POLAR_WEBHOOK_SECRET', '')
